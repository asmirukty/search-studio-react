...
import NewPlaceDialogContent from "./newPlaceDialogContent";
import NewPlaceDialogChip from "./newPlaceDialogChip";
...
export default function NewPlaceDialog() {
    ...
    return (
        <NewStudioDialog open={placeOpen} dialogOpen={placeDialogOpen}
                         handleCancel={placeCancel} handleOk={placeOk}
                         labelCheck={prefectureCityChip.length === 0 && lineStationChip.length === 0}
                         label={'エリア/沿線、駅を選択'}
                         chips={<NewPlaceDialogChip/>}
                         dialogContent={<NewPlaceDialogContent/>}/>
    )
}